...
import { Creature, CreatureType } from "../../types/creatureTypes"
...
const CreatureEditDialog: React.FC<GenericEditDialogProps<Creature>> = ({
  open,
  onClose,
  onSave,
  selectedEntity
  ...
  const [chosenRingId, setChosenRingId] = useState<number | null>(null);
  const [chosenCityId, setChosenCityId] = useState<number | null>(null);

  useEffect(() => {
    if (selectedEntity) {
      setName(selectedEntity.name || "");
      setCoordinateX(selectedEntity.coordinates?.x?.toString() || "");
      setCoordinateY(selectedEntity.coordinates?.y?.toString() || "");
      setAge(selectedEntity.age?.toString() || "");
      setType(selectedEntity.type || CreatureType.HUMAN);
      setAttackLevel(selectedEntity.attackLevel?.toString() || "");
      setChosenCityId(selectedEntity.cityId);
      setChosenRingId(selectedEntity.ringId);
    }
  }, [selectedEntity]);
  

  const handleRingSelection = (ring: Ring) => {
    setChosenRingId(ring.id);
  };
  ...
  const handleSave = () => {
    ...
    if (!chosenRingId || !chosenCityId || !selectedEntity) {
      return;
    }
    ...
    <Dialog open={open} onClose={onClose} fullWidth maxWidth="sm">
      ...
      <DialogContent>
        ...
        <Button
          ...
          sx={{ marginTop: 2 }}
        >
          {chosenRingId ? `Selected Ring Id: ${chosenRingId}` : "Choose a Ring"}
        </Button>
        ...
        <Button
          ...
          sx={{ marginTop: 2 }}
        >
          {chosenCityId ? `Selected City Id: ${chosenCityId}` : "Choose a City"}
        </Button>
      ...
      <DialogActions>
        ...
        <Button disabled={!chosenRingId || !chosenCityId} onClick={handleSave} color="primary" variant="contained">
          Save
        </Button>
      </DialogActions>
    </Dialog>
...
export default CreatureEditDialog;
